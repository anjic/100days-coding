    ...
    this.volume_details_observer$ = this.store.select(getVolumeState).subscribe((data: any ) => {
      if (data && data.hasOwnProperty('result')) {
            ...
            if (volume_data['allocations'].hasOwnProperty('allocations')) {
              ...
              this.resetAllocation();
              for (let i = allocate_cnt - 1; i >= 0; i--) {
                this.hostAllocation.allocated_host.push(volume_data['allocations']['allocations'][i].hostname);
                this.hostAllocation.allocated_id.push(volume_data['allocations']['allocations'][i].globalid);
                this.hostAllocation.allocated_lun.push(volume_data['allocations']['allocations'][i].lun);
              }
            ...
            if (volume_data['allocations'].hasOwnProperty('allocation')) {
                ...
                this.hostAllocation.allocated_host.push(volume_data['allocations']['allocation'].hostname);
                this.hostAllocation.allocated_id.push(volume_data['allocations']['allocation'].globalid);
                this.hostAllocation.allocated_lun.push(volume_data['allocations']['allocation'].lun);
            }
