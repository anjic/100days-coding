...
class ISE_Encryption(APIView):
	...
	def get(self, request, ise_id, format=None):
		...
		get_url = dynamic_url(ise_id, https=True)
		if get_url == status.HTTP_404_NOT_FOUND:
			...
			AUTH = get_url[1]
			url = url_dynamic + RESOURCE['encryption']
	...
	def put(self, request, ise_id, format=None):
		...
		if get_url == status.HTTP_404_NOT_FOUND:
			...
			if payload.has_key('newpasskey'):
				if res['result']['status_code'] == 500:
					(response, status_code) = res_obj.response_formation('Enter Correct Passkey', status.HTTP_400_BAD_REQUEST)
					return Response(response, status=status_code)
